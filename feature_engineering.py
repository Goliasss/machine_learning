...
def main(args):
    ...
    train_data, test_data = sklearn.model_selection.train_test_split(dataset.data, test_size=args.test_size,
    ...
    categoical = np.all(dataset.data.astype(int) == dataset, axis=0)
    continuous = ~np.all(dataset.data.astype(int) == dataset, axis=0)
