...
def main(args):
    ...
    parameteres = {'poly__degree': [1, 2], 'log__C': [0.01, 1, 100], 'log__solver': ['lbfgs', 'sag']}

    grid = sklearn.model_selection.GridSearchCV(pipe, param_grid=parameteres, cv=5)
    grid.fit(train_data, train_target)
    test_predicted = grid.predict(test_data)
